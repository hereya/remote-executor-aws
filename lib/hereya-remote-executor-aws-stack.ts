...
export class HereyaRemoteExecutorAwsStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    userData.addCommands(
      ...
      'fi',
      '',
      '# Restart executor service only if hereya-cli was updated AND the',
      '# executor cgroup has no in-flight job subprocesses. The new binary is',
      '# already on disk; the running executor keeps the old version in memory',
      '# until the next idle window picks up this deferred restart. Stuck jobs',
      '# are bounded by the cloud-side 2-minute heartbeat reaper.',
      'if [ "$HEREYA_UPDATED" = "true" ]; then',
      '  MAIN_PID=$(systemctl show hereya-executor.service -p MainPID --value 2>/dev/null)',
      '  CGROUP=$(systemctl show hereya-executor.service -p ControlGroup --value 2>/dev/null)',
      '  if [ -z "$MAIN_PID" ] || [ "$MAIN_PID" = "0" ] || [ -z "$CGROUP" ]; then',
      '    log "executor not running; restarting to pick up new binary"',
      '    systemctl restart hereya-executor.service 2>&1 | tee -a /var/log/hereya-update.log',
      '  else',
      '    PROC_FILE="/sys/fs/cgroup${CGROUP}/cgroup.procs"',
      '    PROC_COUNT=$(wc -l < "$PROC_FILE" 2>/dev/null || echo 0)',
      '    if [ "$PROC_COUNT" -le 1 ]; then',
      '      log "executor idle (cgroup has $PROC_COUNT proc); restarting for hereya-cli update..."',
      ...
      '      fi',
      '    else',
      '      log "deferring restart: $PROC_COUNT processes in executor cgroup (job in flight); next hourly tick will retry"',
      '    fi',
      '  fi',
      'fi',
      'UPDATEEOF',
